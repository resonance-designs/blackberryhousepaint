    ...
    class Kadence_Status {
        ...
        public static function kt_compileSystemStatus() {
                ...
                if ( is_multisite() ) {
					...
					$all_network_plugins = array();
					if ( is_array( $network_plugins ) && ! empty( $network_plugins ) ) {
						foreach ( $network_plugins as $plugin_path => $plugin_timestamp ) {
							$all_network_plugins[] = $plugin_path;
						}
					}
                    $active_plugins = array_merge( $active_plugins, $all_network_plugins );
                }
