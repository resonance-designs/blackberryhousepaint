...
class Kadence_API_Manager {
	...
	public function kt_api_key_field() {
		...
		if ( ! empty( $this->kt_api_key ) && isset( $this->kt_options[ $this->kt_api_key ] ) && $this->kt_options[ $this->kt_api_key ] ) {
	...
	public function kt_api_email_field() {
		...
		if ( isset( $this->kt_options[ $this->kt_activation_email ] ) && ! empty( $this->kt_options[ $this->kt_activation_email ] ) ) {
			$value = $this->kt_options[ $this->kt_activation_email ];
		}
		echo "<input id='activation_email' name='" . $this->kt_data_key . "[" . $this->kt_activation_email . "]' size='25' type='text' value='" . esc_attr( $value ) . "' />";
		if ( isset( $this->kt_options[ $this->kt_activation_email ] ) && $this->kt_options[ $this->kt_activation_email ] ) {
			echo '<span class="ktap-icon-pos"><i class="dashicons dashicons-yes" style="font-size:20px; color:green;"></i></span>';
		} else {
			echo '<span class="ktap-icon-pos"><i class="dashicons dashicons-warning" style="font-size:20px; color:orange;"></i></span>';
		}
