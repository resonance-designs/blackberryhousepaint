...
function virtue_scripts() {
	...
	if ( isset( $virtue_premium['select2_select'] ) && '1' == $virtue_premium['select2_select'] ) {
		wp_enqueue_script( 'selectWoo', get_template_directory_uri() . '/assets/js/min/select-woo-min.js', array( 'jquery' ), VIRTUE_VERSION, true );
	}
	...
	wp_register_script( 'virtue_google_map_api', 'https://maps.googleapis.com/maps/api/js?key=' . esc_attr( $gmap_api ) . '&callback=initVirtueMap', array( 'virtue_gmap' ), VIRTUE_VERSION, true );
	wp_register_script( 'virtue_gmap', get_template_directory_uri() . '/assets/js/min/virtue-gmap-min.js', array(), VIRTUE_VERSION, true );
