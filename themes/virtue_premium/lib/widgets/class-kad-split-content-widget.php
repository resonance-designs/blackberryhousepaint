...
class kad_split_content_widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$btn_link_target         = ( ! empty( $instance['link_target'] ) ? 'target="' . $instance['link_target'] . '"' : '' );
		...
		echo $before_widget;
			$output = '[kt_imgsplit image="' . $image . '" height="' . $height . '" ' . $img_align . ' ' . $cover . ' ' . $description_max_width . ' ' . $image_id . ' id="' . $args['widget_id'] . '" ' . $img_background_color . ' ' . $img_link . ' ' . $linktarget . ' ' . $content_text_color . ' ' . $description_align . ' ' . $content_background_color . ']';
				if ( ! empty( $title ) ) {
					$output .= '<h2 class="kt_imgsplit_title" style="' . esc_attr( $text_color ) . '">' . $title . '</h2>';
				}
				if ( ! empty( $description ) ) {
					$output .= '<div class="kt_imgsplit_content" style="' . esc_attr( $text_color ) . '">' . $description . '</div>';
				}
				if ( ! empty( $btn_text ) ) {
					$output .= '<a href="' . $btn_link . '" ' . $btn_link_target . ' class="kt_imgsplit_btn kad-btn kad-btn-primary">' . $btn_text . '</a>';
				}
			$output .= '[/kt_imgsplit]';
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['img_cover']                = ! empty( $new_instance['img_cover'] ) ? 1 : 0;
	...
	public function form( $instance ) {
		...
		$cover                     = isset( $instance['img_cover'] ) ? (bool) $instance['img_cover'] : false;
		...
		<div id="virtue_split_content_widget<?php echo esc_attr( $this->get_field_id( 'container' ) ); ?>" class="kad_img_upload_widget kad_infobox_widget">
			...
			<p>
				<input id="<?php echo $this->get_field_id( 'img_cover' ); ?>" name="<?php echo $this->get_field_name( 'img_cover' ); ?>" class="checkbox" type="checkbox"<?php checked( $cover ); ?> />&nbsp;<label for="<?php echo $this->get_field_id( 'img_cover' ); ?>"><?php _e( 'Force image to cover whole area', 'virtue' ); ?></label></p>
			...
			<p><input id="<?php echo $this->get_field_id( 'filter' ); ?>" name="<?php echo $this->get_field_name( 'filter' ); ?>" class="checkbox" type="checkbox"<?php checked( $filter ); ?> />&nbsp;<label for="<?php echo $this->get_field_id( 'filter' ); ?>"><?php _e( 'Automatically add paragraphs', 'virtue' ); ?></label></p>
			<p>
