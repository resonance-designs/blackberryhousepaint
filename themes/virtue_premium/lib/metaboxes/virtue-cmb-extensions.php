 ...
 * @param array  $meta the meta box meta.
 */
 ...
 * @param object $field the meta box field.
 * @param array  $meta the meta box meta.
 * @param number $object_id the meta box ID.
 * @param string $object_type the meta box type.
 * @param object $field_type_object the meta box type object.
 */
...
function virtue_render_select_category( $field, $meta ) {
	...
	$desc = $field->args( 'desc' );
	if ( ! empty( $desc ) ) {
		echo '<p class="cmb_metabox_description">' . wp_kses_post( $desc ) . '</p>';
	}
}
 ...
 */
function virtue_cmb_exclude_templates( $cmb ) {
	$templates_to_exclude = $cmb->prop( 'exclude_templates', array() );
	$excluded             = in_array( get_page_template_slug( $cmb->object_id() ), $templates_to_exclude, true );
	return ! $excluded;
}
