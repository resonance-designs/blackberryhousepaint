...
add_filter( 'template_redirect', 'virtue_ninja_forms_template_override', 5 );
function virtue_ninja_forms_template_override() {
  if ( !empty( $_GET['nf_preview_form'] ) ) {
    get_header();
    ?>
...
add_filter('template_include', 'kad_tec_community_override', 20);
function kad_tec_community_override($template) {
      if ( is_singular() && 'wp_router_page' === get_post_type() ) {
        remove_filter('template_include', array('Kadence_Wrapping', 'wrap'), 101);
      }
      return $template;
}
...
function kt_add_search_form_to_menu($items, $args) {
  global $virtue_premium, $woocommerce;
    ...
    <li class="menu-search-icon-kt">
      <a class="kt-menu-search-btn collapsed" role="button" tabindex="0" title="<?php echo __('Search', 'virtue');?>" data-toggle="collapse" data-target="#kad-menu-search-popup">
