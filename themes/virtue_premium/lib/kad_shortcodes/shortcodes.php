...
function virtue_themecredit_shortcode_function() {
	...
	$output   = '- WordPress Theme by <a target="_blank" rel="nofollow noopener noreferrer" href="' . esc_url( $my_theme->{'Author URI'} ) . '">Kadence WP</a>';
	return $output;
}
include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
if ( is_plugin_active('virtue-toolkit/virtue_toolkit.php') ) {
function virtue_plugin_admin_notice(){
    echo '<div class="error"><p>Please <strong>Disable</strong> the Virtue/Ascend/Pinnacle ToolKit Plugin. It is not needed with Virtue Premium.</p></div>';
}
...
function kadence_youtube_shortcode_function( $atts, $content) {
		...
		// Prepare params
		if ( 'true' == $atts['hideinfo'] ) {
			$atts['showinfo'] = 'false';
		}
		if($atts['hidecontrols'] == 'true') {$atts['controls'] = 'false';}
		foreach ( array('autoplay', 'controls', 'fs', 'modestbranding', 'theme', 'rel', 'loop', 'showinfo' ) as $param ) $params[$param] = str_replace( array( 'false', 'true', 'alt' ), array( '0', '1', '2' ), $atts[ $param ] );
