...
function kad_portfolio_type_shortcode_function( $atts, $content ) {
	...
	if ( $childcategories !== 'false' && $childcategories == true ) {
		$parent = '';
	} else {
		$parent = '0';}
	if ( $columns == '2' ) {
					...
					<div class="portfolio_item grid_item kt_item_fade_in postclass kad-light-gallery kad_portfolio_fade_in">
								...
								if ( ! empty( $ct_image ) ) {
									...
									?>
											<div class="imghoverclass">
												<a href="<?php echo esc_url( get_term_link( $term ) ); ?>" title="<?php echo esc_attr( $term->name ); ?>" class="kt-intrinsic" style="padding-bottom:<?php echo ( $img['height'] / $img['width'] ) * 100; ?>%;">
							  ...
							  <div class="piteminfo">   
								  <h5><?php echo esc_html( $term->name ); ?></h5>
											...
											if ( $showexcerpt == true ) {
												?>
												 <p><?php echo esc_html( $term->description ); ?></p> <?php } ?>
							</div>
						</a>
						</div>
					</div>
